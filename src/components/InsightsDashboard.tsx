...
import { Activity, Layers, ArrowRight, Target, CheckCircle2, GitMerge, ChevronUp, ChevronDown, Minus, AlertTriangle, Brain } from 'lucide-react';
import { MODE_CONFIG, t } from '../constants';
...
const InsightsDashboard: React.FC<Props> = ({ insights, tier }) => { 
  ...
  const latestDistortions = insights.length > 0 ? insights[insights.length - 1].distortions : null;
  const distortionData = latestDistortions ? [
    { name: t('dossier.distortion.allOrNothing.short'), score: latestDistortions.allOrNothing, full: t('dossier.distortion.allOrNothing.full') },
    { name: t('dossier.distortion.catastrophizing.short'), score: latestDistortions.catastrophizing, full: t('dossier.distortion.catastrophizing.full') },
    { name: t('dossier.distortion.emotionalReasoning.short'), score: latestDistortions.emotionalReasoning, full: t('dossier.distortion.emotionalReasoning.full') },
    { name: t('dossier.distortion.shouldStatements.short'), score: latestDistortions.shouldStatements, full: t('dossier.distortion.shouldStatements.full') },
    { name: t('dossier.distortion.personalization.short'), score: latestDistortions.personalization, full: t('dossier.distortion.personalization.full') },
  ] : [];

  const highDistortions = distortionData.filter(d => d.score >= 7);
  ...
  return (
    <div className="space-y-8 p-6 pb-24 overflow-y-auto h-full scrollbar-hide">
      ...
      <div className="flex justify-between items-end border-b border-zinc-800 pb-4">
        <div>
          <h2 className="text-3xl font-light text-white mb-1">{t('dossier.title')}</h2>
          <p className="text-zinc-400 text-sm">{t('dossier.subtitle')}</p>
        </div>
        <div className="text-right">
           <div className="text-2xl font-mono text-emerald-400">{insights.length}</div>
           <div className="text-xs text-zinc-500 uppercase tracking-widest">{t('dossier.reports')}</div>
        </div>
      </div>
      ...
      {metaInsights.length > 0 && (
        <div className="bg-gradient-to-r from-zinc-900 to-zinc-950 p-8 rounded-2xl border border-white/10 relative overflow-hidden group">
          ...
          <div className="flex items-center gap-3 mb-6 relative z-10">
            ...
            <span className="text-sm font-bold uppercase tracking-widest text-white">{t('dossier.metaSynthesisVersion')}</span>
            <div className="ml-auto flex gap-2">
              <span className={`text-xs px-2 py-1 rounded border ${getTypeColor('meta')}`}>{t('dossier.systemWide')}</span>
            </div>
          </div>
          
          <div className="relative z-10 space-y-6">
            <div>
              <h3 className="text-2xl font-light text-white mb-2">{metaInsights[metaInsights.length - 1].title || t('dossier.systemSynthesis')}</h3>
            ...
            {metaInsights[metaInsights.length - 1].crossModelConflicts && metaInsights[metaInsights.length - 1].crossModelConflicts!.length > 0 && (
              <div className="bg-orange-950/20 border border-orange-500/20 p-4 rounded-xl">
                 <div className="flex items-center gap-2 mb-3 text-orange-400">
                    <GitMerge size={16} />
                    <span className="text-xs font-bold uppercase tracking-widest">{t('dossier.activeConflict')}</span>
                 </div>
                 {metaInsights[metaInsights.length - 1].crossModelConflicts!.map((conflict, i) => (
                   <div key={i} className="mb-2 last:mb-0">
                     ...
                     <p className="text-xs text-orange-400 mt-1 flex gap-1"><ArrowRight size={12}/> {t('dossier.resolution')}: {conflict.resolution}</p>
                   </div>
                 ))}
              </div>
            )}
            ...
            <div className="grid md:grid-cols-2 gap-6">
               <div className="space-y-4">
                  <div className="bg-black/30 p-4 rounded-xl border border-white/5 h-full">
                    <div className="text-xs text-emerald-400 uppercase tracking-wider mb-2 flex items-center gap-2"><CheckCircle2 size={12} /> {t('dossier.keyAgreements')}</div>
                    <ul className="list-disc list-inside text-sm text-zinc-300 space-y-1">
                      {metaInsights[metaInsights.length - 1].agreements?.map((a, i) => (
                        <li key={i} className="opacity-80">{a}</li>
                      )) || <span className="text-zinc-500 italic">{t('dossier.noConvergence')}</span>}
                    </ul>
                  </div>
               </div>
               ...
               <div className="space-y-4">
                  {metaInsights[metaInsights.length - 1].rootCause && (
                    <div className="bg-indigo-500/10 p-4 rounded-xl border border-indigo-500/20">
                      <div className="text-xs text-indigo-400 uppercase tracking-wider mb-2 flex items-center gap-2"><Target size={12} /> {t('dossier.rootDriver')}</div>
            ...
            <div className="pt-4 border-t border-white/10 flex justify-between items-center text-sm">
               <span className="text-zinc-500 flex items-center gap-2">{t('dossier.memoryWeight')}: {metaInsights[metaInsights.length - 1].memoryStrength || 100}%</span>
               <span className="text-emerald-400 font-medium">{t('dossier.directive')}: {metaInsights[metaInsights.length - 1].actionableStep}</span>
            </div>
          </div>
        </div>
      )}
      ...
      {distortionData.length > 0 && (
        <div className="bg-aura-card p-6 rounded-2xl border border-zinc-800">
           <div className="flex justify-between items-center mb-6">
             <h3 className="text-xs uppercase tracking-widest text-zinc-500 flex items-center gap-2"><Brain size={14} /> {t('dossier.distortionTrackerTitle')}</h3>
             {highDistortions.length > 0 && (
               <div className="flex items-center gap-2 text-amber-500 text-xs font-bold animate-pulse">
                 <AlertTriangle size={14} />
                 <span>{t('dossier.highDistortionDetected')}</span>
               </div>
             )}
           </div>
           ...
           <div className="h-64 w-full">
             <ResponsiveContainer width="100%" height="100%">
               <BarChart data={distortionData} layout="vertical" margin={{ left: 40, right: 20 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#27272a" horizontal={true} vertical={false} />
                  <XAxis type="number" domain={[0, 10]} hide />
                  <YAxis type="category" dataKey="name" stroke="#71717a" tick={{ fontSize: 10 }} width={80} />
                  <Tooltip 
                    cursor={{ fill: 'rgba(255,255,255,0.05)' }}
                    contentStyle={{ backgroundColor: '#18181b', border: '1px solid #27272a', borderRadius: '8px', color: '#fff' }}
                  />
                  <defs>
                    <linearGradient id="distortionGradient" x1="0" y1="0" x2="1" y2="0">
                      <stop offset="0%" stopColor="#fb923c" />
                      <stop offset="100%" stopColor="#ef4444" />
                    </linearGradient>
                  </defs>
                  <Bar dataKey="score" fill="url(#distortionGradient)" radius={[0, 4, 4, 0]} barSize={20} />
               </BarChart>
             </ResponsiveContainer>
           </div>
           ...
           {highDistortions.length > 0 && (
             <div className="mt-4 p-3 bg-amber-500/10 border border-amber-500/20 rounded-lg">
               <p className="text-xs text-amber-200">
                 <span className="font-bold">{t('dossier.attention')}:</span> {t('dossier.distortionSummaryPrefix')} {highDistortions.map(d => d.full).join(', ')}.
               </p>
             </div>
           )}
        </div>
      ...
      <div className="bg-aura-card p-6 rounded-2xl border border-zinc-800">
        <div className="flex justify-between items-center mb-6">
           <h3 className="text-xs uppercase tracking-widest text-zinc-500 flex items-center gap-2"><Activity size={14} /> {t('dossier.resilienceTrajectoryTitle')}</h3>
        </div>
      ...
      <div className="space-y-6">
        <h3 className="text-xs uppercase tracking-widest text-zinc-500 mb-4 sticky top-0 bg-aura-black py-4 z-10 border-b border-zinc-900">{t('dossier.intelligenceStreamTitle')}</h3>
                ...
                <div className="flex flex-wrap items-center justify-between gap-4 pt-4 border-t border-zinc-800/50">
                   <div className="flex flex-wrap gap-2">
                      {insight.tags?.map((tag, idx) => (
                        <span key={idx} className="text-[10px] text-zinc-500 bg-zinc-900 px-2 py-1 rounded border border-zinc-800">#{tag}</span>
                      ))}
                   </div>
                   <div className="flex items-center gap-2 text-sm">
                      <span className="text-zinc-500 text-xs uppercase tracking-wide">{t('dossier.nextStep')}:</span>
